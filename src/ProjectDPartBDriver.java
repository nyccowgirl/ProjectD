...
public class ProjectDPartBDriver {
	...
	private static BinarySearchTreeWithDups<PoliceReport> buildTreeFromList(List<PoliceReport> list) {
		...
		try {
			...
			System.out.println("\tLeft height: ***Cannot find the left height! Left subtree too deep! Stack overflow.");
		}
		try {
			System.out.println("\tRight height: " + reportTree.getRightHeight());
		} catch(StackOverflowError ex) {
			System.out.println("\tRight height: ***Cannot find the right height! Right subtree too deep! Stack overflow.");
		}
